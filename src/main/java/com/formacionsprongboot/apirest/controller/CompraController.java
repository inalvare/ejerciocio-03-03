...
public class CompraController {
	...
	@GetMapping("/fechaCompras/{id}")
	public ResponseEntity<?>  findCompraById(@PathVariable Date fecha) {
		Compra compra = null;
		List<Compra> lista=servicio.findAll();
		Map<String, Object> response = new HashMap<>();
		
		try {
			for (Long i = (long) 0; i < lista.size(); i++) {
			compra=servicio.findById(i);
			if(fecha==compra.getFecha()) break;
		}
